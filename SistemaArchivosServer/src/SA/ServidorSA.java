...
public class ServidorSA {
    ...
    /// Exportar Archivo
     ...
     public String exportarArchivo(String pnombreArchivo){
        ...
        if(indexcontroldisco >
        _estructuraDisco.getListaArchivos().size() || indexcontroldisco <0)
        {
            return "Error de archivo";
        }
        else{
        Archivo _archivo =
        _estructuraDisco.getListaArchivos().get(indexcontroldisco);
        mensajeRetorno =
        _accesoDatos.leerArchivo(_archivo.getByteInicio(),_archivo.getEspacioAsignado(),
        _estructuraDisco);
